...
sfc_3 = SFC(np.array([[min_len_3+4],[3],[5]]), T3, R3)
...
point_2 = np.array([[7],[10],[0]])
point_3 = np.array([[14],[7],[8]])
point_4 = np.array([[20],[14],[8]])
point_5 = np.array([[25],[5],[3]])
...
sfc_1 = SFC(np.array([[min_len_1+5],[3],[4]]), T1, R1)
sfc_2 = SFC(np.array([[min_len_2+3],[4],[2]]), T2, R2)
sfc_3 = SFC(np.array([[min_len_3+5],[3],[3]]), T3, R3)
sfc_4 = SFC(np.array([[min_len_4+5],[4],[3]]), T4, R4)
...
point_5 = np.array([[7],[-1],[8]])
point_6 = np.array([[15],[-2],[8]])
...
sfc_1 = SFC(np.array([[min_len_1+4],[3],[3]]), T1, R1)
sfc_2 = SFC(np.array([[min_len_2+3],[4],[2]]), T2, R2)
sfc_3 = SFC(np.array([[min_len_3+4],[3],[3]]), T3, R3)
sfc_4 = SFC(np.array([[min_len_4+3],[3],[2]]), T4, R4)
...
for i in range(len(waypoint_data_list)):
    ...
    for j in range(len(sfc_list_all[i])):
        ...
        if i == 2:
            text_level = -15
        ax.text(-10,-10,text_level,"evaluation time: " + str(np.round(eval_time,2))+ "\n num ctrl pts: " + str(num_cont_pts))
        ...
        if j == 0:
            ...
            alpha = 0.65
            if i == 0:
                ax.set_title("With SFC Constraints")
