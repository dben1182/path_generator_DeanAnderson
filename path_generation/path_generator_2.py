...
from PathObjectivesAndConstraints.python_wrappers.waypoint_constraints_old import WaypointConstraints
...
from path_generation.waypoint_data import Waypoint
import time
...
# when minimizing acceleration, no max velocity constraint needed
    ...
    def __get_constraints(self, num_cont_pts: int, intervals_per_corridor: tuple, point_sequence: np.ndarray, 
        ...
        if waypoint_directions is not None:
                direction_constraint = self.__create_waypoint_direction_constraint(waypoint_directions, num_cont_pts)
        ...
        if waypoint_accelerations is not None:
            ...
            if np.sum(accel_mags) > 0.000000001:
                ...
                if accel_mags[1] > 0.0000000001:
                    switches[1] = True
                waypoint_accel_constraint = self.__create_waypoint_unscaled_acceleration_constraint(waypoint_accelerations, num_cont_pts, switches)
...
# bounds over just the scale factor
    ...
    def __create_waypoint_direction_constraint(self, directions, num_cont_pts):
        lower_bound = 0
        upper_bound = 0
        desired_angles_start = self.__get_direction_angles_from_vector(directions[:,0])
        desired_angles_end = self.__get_direction_angles_from_vector(directions[:,1])
        def direction_constraint_function(variables):
            control_points = np.reshape(variables[0:num_cont_pts*self._dimension], \
            (self._dimension,num_cont_pts))
            angles_start = self.__get_direction_angles_from_cont_pts(control_points,"start")
            angles_end = self.__get_direction_angles_from_cont_pts(control_points,"end")
            constraints = np.concatenate(((angles_start - desired_angles_start).flatten(),
                                          (angles_end -desired_angles_end).flatten())) 
            return constraints
    ...
    def __get_direction_angles_from_cont_pts(self, control_points, terminal_point = "start"):
        direction_vector = np.array([])
        if terminal_point == "start":
            direction_vector = control_points[:,2] - control_points[:,0]
        else: # terminal_point == "end"
            direction_vector = control_points[:,-1] - control_points[:,-3]
        return self.__get_direction_angles_from_vector(direction_vector)
        
        
    def __get_direction_angles_from_vector(self, direction_vector):
        if self._dimension == 2:
            return self.__get_2D_direction_angle(direction_vector)
        else: #dimension = 3
            return self.__get_3D_direction_angles(direction_vector)
        
        
    def __get_2D_direction_angle(self, direction_vector):
        # returns angle rotation required to point into the direction vector
        # starting from the x direction. Rotation about Z axis (psi)
        dx = direction_vector.item(0)
        dy = direction_vector.item(0)
        psi = np.arctan2(dy,dx)
        return np.array([psi])
    
    def __get_3D_direction_angles(self, direction_vector):
        # returns angle rotations required to point into the direction vector
        # starting from the x direction. Rotation about Z axis (psi), rotation
        # rotation about the y axis (theta)
        dx = direction_vector.item(0)
        dz = direction_vector.item(2)
        ...
        Ry = np.array([[c_theta, 0, s_theta],
                    [0, 1, 0],
                    [-s_theta, 0, c_theta]])
        direction_vector_2 = Ry @ direction_vector
        dx_2 = direction_vector_2.item(0)
        dy_2 = direction_vector_2.item(1)
        psi = np.arctan2(dy_2,dx_2)
        return np.array([psi, -theta])

    
    def __create_waypoint_unscaled_acceleration_constraint(self, waypoint_unscaled_accelerations, num_cont_pts, switches):
        lower_bound = 0
        upper_bound = 0
        def acceleration_constraint_function(variables):
            ...
            constraints = self._waypoint_const_obj.acceleration_at_waypoints_constraints(control_points,
                scale_factor, waypoint_unscaled_accelerations, switches)
