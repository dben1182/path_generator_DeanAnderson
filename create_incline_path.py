...
waypoint_2 = Waypoint(location=np.array([[0],[350],[-400]]))
...
print("max incline angle" , np.arctan(np.max(incline_data))*180/np.pi)
print("min incline angle" , np.arctan(np.min(incline_data))*180/np.pi)
